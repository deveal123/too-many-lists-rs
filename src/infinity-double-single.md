...
그리고 `push` 랑 `pop` 놈들만 살짝 마개조(rework) 해줍시다:
...
여기서 `boolean` 놈들을 뱉어내는 건 걍 우리가 실제로 한 발짝 걷는 데(move) 성공했는지 편하게 눈치채려고 덤으로 달아둔 겁니다. 자, 이제 이 기염을 토하는 괴물(baby)을 테스트해 봅시다:
...
우리가 가리키고 있는 손가락 주변에선 빛의 속도로(very fast) 리스트 내용을 갈아엎을 수 있지만, 만약 손가락에서 존나게 멀리 떨어진 외곽 지역을 건드리고 싶다면 걍 거기까지 뚜벅뚜벅 직접 걸어가야만(walk all the way) 합니다.
