...
여기서 다시 한번, 안전성(safety)이 상태 의존적(stateful)이라는 사실을 확인할 수 있습니다. 만약 우리가 *이 함수 내에서(in this function)* 제거된 꼬리 포인터를 제대로 초기화(null out)하지 않는다면 당장은 아무런 문제도 발생하지 않습니다(no problems at all). 하지만 바로 그다음 `push`가 연달아 호출되는(subsequent calls to `push`) 순간, 시스템은 해제되어 허공을 가리키는 댕글링 포인터(the dangling tail)를 향해 값을 마구 쓰기 시작할 겁니다(start writing)!
...
mod test {
    ...
    fn basics() {
        ...
        // 꼬리가 완벽히 제거된 이후에도 꼬리 포인터 상태가 올바르게 초기화(fixed the pointer right)되었는지 확인
        list.push(6);
        list.push(7);

        // 마지막 정상 추출 및 제거(normal removal) 작동 검토
...
이건 사실 예전 단방향 리스트 시절의 스택 테스트를 거의 그대로 복제한 것이지만, 큐의 특성상 `pop`으로 도출되는 결과의 순서가 반대로 뒤집혀(flipped around) 있다는 점이 다릅니다. 추가로, 마지막 단계에는 `pop` 중 발생할지도 모를 꼬리 포인터 오류(tail-pointer corruption)를 막기 위한 간이 점검 과정(extra steps)을 하나 더 추가했습니다.
