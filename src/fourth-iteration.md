...
어쩌면 우리가... 저 Rc를 맵핑해서... `Rc<T>`를 얻어내는 게 가능할지도 몰라요? 그런 기능이 있나요? Rc 공식 문서를 죽을봐도 그런 기능은 없는 것 같습니다. (사실 어떤 용자가 그런 정신 나간 짓을 가능하게 해주는 [크레이트][own-ref]를 따로 만들긴 했습니다.)
