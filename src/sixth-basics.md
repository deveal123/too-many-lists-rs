...
좋습니다, 이제 이 책에서 제일 짜증 나는 파트이자, 제가 이 챕터 하나 쓰는 데 무려 7년이나 걸린 이유에 도달했습니다! 우리가 이미 5번이나 우려먹었던 진짜 개노잼 작업들을 또다시 불태울 시간인데, 이번엔 심지어 모든 걸 두 배로 길고 번거롭게(extra verbose and long) 처리해야 합니다. 왜냐하면 모든 걸 앞뒤 두 번씩 처리해야 하고, 게다가 그놈의 `Option<NonNull<Node<T>>>` 포장지까지 일일이 다 까야 하니까요!
...
아 맞다 씨발, 전 진짜 이 포인터 찌꺼기 자식 새끼들이 너무 혐오스럽습니다. 우리는 NonNull 껍데기에서 원시 포인터를 끄집어내기 위해 `as_ptr`을 명시적으로 일일이 박아줘야 합니다. 왜냐하면 저놈의 DerefMut이라는 마법이 빌어먹을 `&mut`을 기반으로 정의되어 있는데, 우린 안전하지 않은 코드 구역 안에 함부로 그 안전한 참조자(safe references) 나부랭이들을 마구잡이로 풀어놓고 싶지 않기 때문입니다!
